...
async def on_member_join(member):
    ...
    channel = discord.utils.get(member.guild.channels, id=603156893355212800)
            ...
            for embed in message.embeds:
                if embed.title:
                    ...
                    if message.channel.name.startswith("初心者tao🌸"):
                        level = int(message.channel.name.split("-")[-1])
                        setting = f"{level}"
                ...
                if embed.title:
                    ...
                    if channel is not None:
                        if title.find("が待ち構えている...！"):
                            ...
                            if title.find("【超激レア】") != -1 or title.find("【レア】") != -1 or title.find("【激レア】") != -1:
                                ...
                                await channel.send(embed=embed,*f"{role.mention}\n{name}が出現したよ")
